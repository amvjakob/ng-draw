...
export class FreehandDrawingDirective implements AfterViewInit, OnChanges, OnDestroy
{
  ...
  // touch handlers
  protected _touchDown: (evt: TouchEvent) => void;
  protected _touchUp: (evt: TouchEvent) => void;
  protected _touchMove: (evt: TouchEvent) => void;
  ...
  constructor(protected _elRef: ElementRef)
  {
    ...
    // square with min dimension
    const dim = Math.min(window.innerWidth, window.innerHeight - 64); // -64 to remove toolbar height (kinda dirty)
    const options = Object.assign({width: dim, height: dim},
  ...
  public eraseStroke(index: number): boolean
  {
    if (index < this._strokes.length)
    {
      ...
      if (this._strokeContainer.children.indexOf(s) == index) {
  ...
  public exportAsImage(): void {
    let renderer = this._app.renderer;
    renderer.render(this._app.stage);
    let data = renderer.view.toDataURL();
  ...
  protected __assignMouseHandlers(): void
  {
    ...
    // add support for touch moves
    this._touchDown = (evt: TouchEvent) => 
      this.beginStrokeAt(evt.touches[0].clientX - this._rect.left, evt.touches[0].clientY - this._rect.top);
    this._touchUp = (evt: TouchEvent) => 
      this.endStrokeAt(evt.changedTouches[0].clientX - this._rect.left, evt.changedTouches[0].clientY - this._rect.top);
    this._touchMove = (evt: TouchEvent) => 
      this.updateStroke(evt.touches[0].clientX - this._rect.left, evt.touches[0].clientY - this._rect.top);

    this._container.addEventListener('touchstart', this._touchDown, { passive: true });
    this._container.addEventListener('touchend', this._touchUp, { passive: true });
    this._container.addEventListener('touchmove', this._touchMove, { passive: true });
